...
const createBracket = teams => {
  ...
  let rounds = 0;
  let countTeams = copyTeams.length; //count rounds necessary for the whole bracket
...
const insertScore = (user, score, bracket) => {
  const bracketCopy = copyBracket(bracket); //searching for the last game of the current team

  const match = findMatchByTeamID(user, bracket); //inserting the score in the bracket
